...
namespace Spaanjaars.FileProviders.FileSystem
{
  ...
  public class FileSystemFileProvider : IFileProvider
  {
    ...
    public Task<List<FileInfo>> GetFilesAsync(string rootContainer)
    {
      ...
      {
        var relativePath = Regex.Replace(file.FullName, Regex.Escape(folder), "", RegexOptions.IgnoreCase);
    ...
    public Task<byte[]> GetFileAsync(string rootContainer, string filePath)
    {
      var folder = GetFolderPath(rootContainer);
      ThrowIfFolderNotExists(folder);
    ...
    public Task StoreFileAsync(string rootContainer, string filePath, byte[] fileBytes, bool overwrite)
    {
      var folder = GetFolderPath(rootContainer);
      var file = GetFilePath(folder, filePath);
    ...
    private static void ThrowIfFolderNotExists(string folder)
    {
    ...
    private static void ThrowIfFileNotExists(string file)
    {
      if (!File.Exists(file))
      {
    ...
    {
      if (!rootContainer.EndsWith(Path.DirectorySeparatorChar.ToString()) && !rootContainer.EndsWith(Path.AltDirectorySeparatorChar.ToString()))
      {
    ...
    private static string GetFilePath(string folder, string filePath)
    {
