...
namespace Spaanjaars.Email.Smtp
{
  /// <summary>
  ...
  public class SmtpMailSender : IMailSender
  {
    ...
    /// <summary>
    /// Creates a new instance of the SmtpMailSender class.
    /// </summary>
    /// <param name="emailSettings">A collection of settings used to connect to a remote SMTP server.</param>
    public SmtpMailSender(IOptions<List<SmtpSettings>> emailSettings)
    {
    ...
    private static async Task<bool> SendMailAsync(SmtpSettings smtpSettings, MailMessage mailMessage)
    {
      ...
      catch (SmtpException smtpEx)
      {
        switch (smtpEx.StatusCode)
        {
          case SmtpStatusCode.MailboxBusy: // Sleep for a bit and then retry. // If it fails again, try the alternative server.
            return await Retry(mailMessage, client);
    ...
    private static async Task<bool> Retry(MailMessage mailMessage, SmtpClient client)
    {
